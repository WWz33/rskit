...
from rnaseq_tools.core.base import ToolBase
from rnaseq_tools.config import StarConfig
from rnaseq_tools.utils.validators import validate_file, check_star_index

class StarIndexer:
    def __init__(self, config: StarConfig):
        self.config = config
        self.tool = ToolBase("STAR")
...
class StarAligner:
    ...
    def align(self, index_dir: str, fq1: str, fq2: str, output_prefix: str, 
        ...
        cmd = ["STAR", "--runThreadN", str(self.config.threads), "--genomeDir", index_dir,
               "--readFilesIn", fq1, fq2, "--readFilesCommand", "cat",
